...
function call_multiwindow(json, loadFromApi = false) {
    ...
    tradingwindow.registerComponent('widget', function (container, state) {
        ...
        // console.log(container.getElement().html(state.content))
            ...
            tab
                ...
                .on('click', (function (e) {
                    ...
                    $(this).parents('.lm_header:eq(0)').siblings('.lm_items').find('.lm_item_container').each(function (i, obj) {
                        ...
                        if ($(this).find('.web-time-sales-table').length > 0) {
                           unscribeToTimeSalesChannel();
                        }
    ...
    tradingwindow.on( 'stackCreated', function( stack ){
    ...
    if (loadFromApi) { 
        updatesWidgetContensFromApi(tradingwindow); 
    }
    ...
    // Container State changed but to widget load/unload
            ...
            if (widgetTitle.includes('Watchlist')) { 
                ...
                (function (index) {
                    $.get('shared/widget_time_sale_ticker/', function (data) { 
                        updateWidgetContent(widgetsToUpdate[index].container, data);
                        // subscribeUnsubscribeTickerChannel(true);
                    });
                })(i);
            }
...
function saveGrid(pid, profile_page) {
    if (profile_page != null) { 
        ...
        var data = {
            ...
            pdata: 'localstorage',
            pid: pid,
            profile_content_tradeportal: state,
            overwrite: '1'
        };
...
function loadGrid(pid, profile_page) {
    if (profile_page != null) { 
        $.get('/shared/loadprofile/', { profile: pid })
            .done(function (data) {
                var tradingLayout = data.profile_content_tradeportal;
                if (tradingLayout !== null && tradingLayout !== undefined) {  
                    call_multiwindow(JSON.parse(tradingLayout), true); 
                } else { 
                    //Default Content for Trading Portal
                    $.getJSON("static/default_content/trading_default_content.json", function (json) {call_multiwindow(json, true);});
                }
...
$('#tradingContainer').on('orderTerminalCodeChanged','.order_terminal_code', function () {
    let terminal_id = $(this).data('terminal');
    get_order_history(terminal_id); 
})

$('#tradingContainer').on('orderTerminalSymbolChanged', '.order_terminal_symbol', function () {
    let terminal_id = $(this).data('terminal');
    get_order_history(terminal_id);
})

$('#tradingContainer').on('click', '.wd_settings', function () {   
