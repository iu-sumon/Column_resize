...
function updateRealtimeOrderData(msg) {
    console.log('updateRealtimeOrderData', msg);
    // Constants
    ...
    if (msg.order_side === 'BUY') {
        ...
        if (msg.exec_status === 'Trade Executed') {
            if ($(`.${msg.order_symbol}_${msg.cc}_total_qty`).length) {
                updateExistingPortfolio(msg, 'BUY');
            } else {
    ...
    if (msg.order_side === 'SELL') { 
        ...
        if (UPD_STATS.includes(msg.order_status) && UPD_STATS.includes(msg.exec_status)) { 
            updateQty(msg.order_symbol, msg.cc, -msg.q);
        }

        if (BACK_STATS.includes(msg.order_status) && BACK_STATS.includes(msg.exec_status)) {
            updateQty(msg.order_symbol, msg.cc, (msg.q - msg.cq));
        }
        
        if (msg.exec_status === 'Trade Executed') {

            let execQty = parseFloat(msg.exec_qty);  
            updateExistingPortfolio(msg, 'SELL');
...
function updateCash(clientCd, amt) {
    let cashEl = $(`.cash_${clientCd}`);
    let currCash = getNumVal(cashEl);
    cashEl.text((currCash + amt).toFixed(1));
};
...
function updateExistingPortfolio(msg,side) {
    console.log('updateExistingPortfolio', msg, side);
    ...
    console.log('newGainPct', newGainPct);
    console.log('newInvestPct', newInvestPct);
